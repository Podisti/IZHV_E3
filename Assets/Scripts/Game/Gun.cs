...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun)
        {
            var bulletAngleDif = shotgunSpread/shotgunBullets;

            for (int i = 0; i < shotgunBullets; i++)
            {
                Quaternion bulletRotation = Quaternion.Euler(director.rotation.eulerAngles.x, director.rotation.eulerAngles.y, director.rotation.eulerAngles.z + (-(shotgunSpread/2) + (i * bulletAngleDif)));

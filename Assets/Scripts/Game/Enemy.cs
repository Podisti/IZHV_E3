...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        {
            var destination = nearestPlayer.transform.position;
    
            var direction = destination - transform.position;
            direction.Normalize();
            var move = direction/50 * speed; // divided by 50 to slow the movement


         
            mRigidBody.MovePosition(transform.position + move);
